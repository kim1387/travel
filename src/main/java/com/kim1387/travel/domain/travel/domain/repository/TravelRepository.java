...
import com.kim1387.travel.domain.travel.domain.repository.querydsl.TravelRepositoryQuerydsl;
...
public interface TravelRepository extends JpaRepository<Travel, Long>, TravelRepositoryQuerydsl {
  ...
  @Query(
      value =
          "select t from Travel t join fetch t.users join fetch t.city where t.id = :id and"
              + " t.isActivated = true")
